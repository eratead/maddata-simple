...
**Status (2026-08-19):** Phases 1 and 2 built and deployed (`0fb7a34`). **Phases 3 and 4 built** to [health-monitor-phases-3-4.md](../specs/health-monitor-phases-3-4.md) — not yet deployed. Full suite 807 passing.
Phase 4's first real run found **2 critical and 13 high** composer advisories against the deployed lock (see HM-4.3) — a genuine finding, not a false positive; the matches were verified by hand against Packagist.
...
- [ ] **HM-3.11** ~~Update `docs/architecture_map.md` §18~~ *(done — new "Admin surface (Phase 3)" subsection)*. Remaining: **deploy** staging → production, code-only (`git push origin main` first, then `main:staging`).
...
- [ ] **HM-4.12** *(docs done — runbook "Dependency currency (Phase 4)", architecture_map §18, parent spec §10. Remaining: the two server-side steps, which need droplet access.)* Ops + docs: enable `unattended-upgrades` on production (**security pocket only**, `Automatic-Reboot "false"`, keep a `.bak`) — provisioning, never the deploy flow. Redeploy the facts script (HM-4.5). Update `docs/runbooks/health-monitor.md`, `docs/architecture_map.md` §18, and the parent spec's §3 catalog + §10 phasing table.
