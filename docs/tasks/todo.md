...
- `d4` — never recorded, and staying that way deliberately: recording a patch run nobody performed would be a false entry in the one check that answers "when did a human last patch?". It clears when the `d1` backlog is actually addressed.
- ~~`S2a` / `S2b` / `B4` — "has never completed" / "never recorded".~~ **These were false, and self-inflicted.** Markers shared the app cache store, and `php artisan cache:clear` — which every deploy runs — was erasing them; four deploys in one day made healthy jobs look like they had never run. Same class of mistake as keeping the backup marker on tmpfs where a reboot ate it. Markers now live in a dedicated `health` file store (`eccc9d4`), which also removes their dependency on MySQL and Redis, two of the things being monitored. `B4` was re-recorded at the real drill date via the new `--at` option rather than stamped "now".
- ~~`X1` — expired Sanctum tokens.~~ Nothing was ever deleting them, so the count could only grow. `sanctum:prune-expired` now runs weekly.
