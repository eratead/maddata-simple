...
@endpush
    ...
    <form id="editAgencyUserForm" method="POST" action="{{ route('agency.users.update', [$agency, $user]) }}"
          x-data="{
              accessMode: '{{ old('access_all_clients', $accessAllClients ? '1' : '0') }}',
              selectedClients: @js(old('clients', $assignedClientIds)),
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="pt-4 border-t border-gray-100">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                ...
                                <span class="ml-3 text-sm text-gray-700">Account Active</span>
                            </label>
                            <p class="mt-1 text-xs text-gray-400">Disabled users cannot log in. All pivot data is preserved.</p>
                            @if(auth()->id() === $user->id)
                                <p class="mt-1 text-xs text-amber-500">You cannot change your own status.</p>
                            @endif
                        </div>
                    </div>
                </x-page-box>
        ...
        <div class="mt-6 pt-5 border-t border-gray-200 flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
            ...
            <div class="flex items-center gap-3 w-full sm:w-auto">
                @unless (auth()->id() === $user->id)
                    @if($user->is_active)
                        <button type="button"
                                @click="$dispatch('confirm-action', {
                                    title:        'Disable user?',
                                    message:      @js($user->name) + ' will be disabled and unable to log in.',
                                    confirmLabel: 'Disable',
                                    form:         document.getElementById('disable-form')
                                })"
                    ...
                    @else
                        <span class="text-xs font-medium text-gray-500 px-3 py-2 border border-gray-200 rounded-lg bg-gray-50">
                            User is currently disabled. Toggle "Account Active" above to re-enable.
                        </span>
                    @endif
                @else
    ...
    {{-- Hidden disable form --}}
    @unless (auth()->id() === $user->id)
        <form id="disable-form" action="{{ route('agency.users.destroy', [$agency, $user]) }}" method="POST" class="hidden">
            @csrf @method('DELETE')
        </form>
    @endunless
