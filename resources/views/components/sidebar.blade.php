...
<aside class="fixed inset-y-0 left-0 z-30 w-60 flex flex-col bg-[#111827] transition-transform duration-300 lg:relative lg:translate-x-0 shrink-0"
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5" aria-label="Main navigation">
        ...
        @if (Auth::user() && (Auth::user()->hasPermission('is_admin') || Auth::user()->hasPermission('can_see_logs')))
            ...
            @if($sidebarIsAdmin)
                ...
                {{-- System Health (admin only) — the read-only sibling of the
                     control page above. --}}
                <a href="{{ route('admin.monitor.index') }}"
                   class="flex items-center gap-2.5 px-3 py-2 rounded-md text-sm font-medium transition-colors {{ request()->is('admin/monitor*') ? 'nav-active' : 'text-slate-400 hover:bg-white/[0.08] hover:text-white' }}">
                    {{-- Flowbite: general/heart-pulse --}}
                    <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12h4l2-5 3 10 2.5-5H21"/>
                    </svg>
                    System Health
                </a>
            @endif
    ...
    <div x-data="{ open: false }" class="border-t border-white/10 px-3 py-3 shrink-0">
        ...
        <a href="{{ route('profile.edit') }}"
           class="flex items-center gap-2.5 px-3 py-2 rounded-md text-sm font-medium transition-colors {{ request()->routeIs('profile.*') ? 'nav-active' : 'text-slate-400 hover:bg-white/[0.08] hover:text-white' }}">
        ...
        <a href="{{ route('settings.sign-in-methods.index') }}"
           class="flex items-center gap-2.5 px-3 py-2 rounded-md text-sm font-medium transition-colors {{ request()->routeIs('settings.sign-in-methods.*') ? 'nav-active' : 'text-slate-400 hover:bg-white/[0.08] hover:text-white' }}">
            {{-- Flowbite: general/shield --}}
            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
