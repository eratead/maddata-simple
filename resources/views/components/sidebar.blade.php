                ...
                @if (Auth::user() && Auth::user()->hasPermission('is_admin'))
                        <div x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
                                <x-dropdown align="left" width="48" x-bind:open="open">
                                        <x-slot name="trigger">
                                                <button type="button"
                                                        class="inline-flex items-center pl-5 py-2 border border-transparent text-sm font-medium rounded-md text-gray-500 w-full text-center {{ request()->is('clients*') || request()->is('users*') || request()->is('admin/activity-logs*') ? 'bg-gray-100' : 'hover:text-gray-700' }} transition">
